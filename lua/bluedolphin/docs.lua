...
local Groups = require("bluedolphin.groups")
...
function M.extras()
  local Extra = require("bluedolphin.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/bluedolphin/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/bluedolphin/config.lua", "\n(--@class bluedolphin%.Config.-\n})")
